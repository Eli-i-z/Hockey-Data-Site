...
cleanedPlayerData = playerData.dropna(subset=['goals', 'assists'])

def getDefensiveGoals():
    defensivePlayers = cleanedPlayerData[cleanedPlayerData.iloc[:, 6] == "D"]
    defensiveGoals = defensivePlayers.iloc[:, 11].sum()
    return defensiveGoals
def getOffensiveGoals():
    offensivePlayers = cleanedPlayerData[cleanedPlayerData.iloc[:, 6] != "D"]
    offensiveGoals = offensivePlayers.iloc[:, 11].sum()
    return offensiveGoals
def getOffensivePlayers():
     offensivePlayers = len(cleanedPlayerData[playerData.iloc[:, 6] != "D"])
     return offensivePlayers
def getDefensivePlayers():
     defensivePlayers = len(cleanedPlayerData[cleanedPlayerData.iloc[:, 6] == "D"])
